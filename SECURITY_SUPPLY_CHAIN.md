...
This repo treats dependency, workflow, release, and generated-client changes as security-sensitive.
...
- Release jobs must not run on `pull_request`, `pull_request_target`, `discussion`, or other user-content events.
- Require review for `.github/workflows/`, `.github/actions/`, package manifests, lockfiles, `.npmrc`, `scripts/`, release config, and generated-client code.
...
If a compromised package install ran: assume local/user secrets are burned, revoke npm/GitHub/cloud tokens, rotate deploy secrets, compare release artifacts against source, and rebuild the machine instead of surgical cleanup theater.
